...
describe('Hyperons', () => {
  describe('elements', () => {
    it('renders simple elements', () => {
      ...
      expect(result.toString()).to.equal('<div></div>')
    })

    it('renders void elements', () => {
      const result = subject('br')
      expect(result.toString()).to.equal('<br/>')
    })
    ...
    it('invokes higher-order components', () => {
      ...
      expect(result.toString()).to.equal('<span class="hoc"></span>')
    })
  })
  ...
  describe('properties', () => {
    it('renders HTML attribute names and values', () => {
      ...
      expect(result.toString()).to.equal('<form action="/submit" method="post"></form>')
    })
    ...
    it('aliases conflicting JS keywords', () => {
      ...
      expect(result.toString()).to.equal('<label class="label" for="id"></label>')
    })

    it('escapes HTML attribute values', () => {
      const result = subject('img', { alt: '"Mac & Cheese"' })
      expect(result.toString()).to.equal('<img alt="&quot;Mac &amp; Cheese&quot;"/>')
    })
    ...
    context('styles', () => {
      it('stringifies style attributes', () => {
        ...
        expect(result.toString()).to.equal('<div style="padding:0.5em 1em;margin:1em 0;"></div>')
      })

      it('hyphenates style properties', () => {
        const result = subject('div', { style: { paddingTop: '0.5em', marginBottom: '1em' } })
        expect(result.toString()).to.equal(
          '<div style="padding-top:0.5em;margin-bottom:1em;"></div>'
        )
      })
  ...
  describe('children', () => {
    ...
    it('renders text children', () => {
      ...
      expect(result.toString()).to.equal('<div>Hello World</div>')
    })

    it('renders numeric children', () => {
      const result = subject('div', null, 123)
      expect(result.toString()).to.equal('<div>123</div>')
    })

    it('escapes text children', () => {
      const result = subject('div', null, '"Mac & Cheese"')
      expect(result.toString()).to.equal('<div>&quot;Mac &amp; Cheese&quot;</div>')
    })

    it('does not escape HTML children', () => {
      const result = subject('div', null, subject('i', null, 'Hello World'))
      expect(result.toString()).to.equal('<div><i>Hello World</i></div>')
    })
    ...
    it('renders multiple children', () => {
      ...
      expect(result.toString()).to.equal('<div><i>Hello</i> <i>World</i>!</div>')
    })
    ...
    it('passes children to higher-order components', () => {
      ...
      expect(result.toString()).to.equal('<ul><li>one</li><li>two</li></ul>')
    })
